...
export const RiichiMahjongInputScreen = ({ navigation, route }: RiichiMahjongInputScreenProps) => {
  ...
  const NextButton = () => (
    <MarginButton status='info' appearance='outline' onPress={() => {
  ...
  const CompleteButton = () => (
    <MarginButton status='success' onPress={() => {
      handInformations.push(handString);
      handInformationsUpdate(handInformations);
  ...
  return <Main>
    <MarginInput disabled={inputLevel > 0} placeholder='손패 (후로 제외)' onChangeText={changeUpdte} />
    {inputLevel >= 1 && <FadeInView style={{}}><MarginInput placeholder='후로패' disabled={inputLevel > 1} onChangeText={changeUpdte} /></FadeInView>}
    {inputLevel >= 2 && <FadeInView style={{}}><MarginInput placeholder='도라표지패' disabled={inputLevel > 2} onChangeText={changeUpdte} /></FadeInView>}
    {inputLevel >= 3 && <FadeInView style={{}}><MarginInput placeholder='화료패(론, 쯔모 선언패)' disabled={inputLevel > 3} onChangeText={changeUpdte} /></FadeInView>}
    <HandImage handString={handString} />
    <MarginButton status='basic' accessoryLeft={PictureIcon}>
      사진으로 입력
    </MarginButton>
    {inputLevel === 3 ? <CompleteButton /> : <NextButton />}
    {inputLevel >= 1 && <MarginButton status='danger' appearance='outline' onPress={() => {
      inputLevelUpdate(inputLevel - 1);
